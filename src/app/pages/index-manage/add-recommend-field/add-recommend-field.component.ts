...
@Component({
    selector: 'ngx-add-recommend-field',
    templateUrl: './add-recommend-field.component.html',
    styleUrls: ['./add-recommend-field.component.scss'],
})
export class AddRecommendFieldComponent implements OnInit {
    ...
    ngOnInit() {
        if (this.rowData) {
            ...
            //     action: 'getRecommendCommodityDetail',
            //     id: this.rowData.id,
            // }).then(res => {
            //     if (res['serviceCall']) {
    ...
    onSubmit(e) {
        ...
        if (this.rowData) {
            ...
            this.imageUpload.uploadEntrance(this.entrance, sendData).then(data => {
                data['action'] = 'editRecommendCommodity';
                sendData['goodsRecommendedId'] = this.rowData.recommendGoodId;
            ...
            //     } else {
            //         sendData['action'] = 'editRecommendCommodity';
            //         sendData['goodsRecommendedId'] = this.rowData.recommendGoodId;
            ...
            // if (this.entrance[0].showFileList.length !== 0) {
            this.imageUpload.uploadEntrance(this.entrance, sendData).then(data => {
                data['action'] = 'addRecommendCommodity';
                this.dataService.sendRequest(data).then(res => {
                    if (res['serviceCall']) {
                        const callData = this.dataService.getCallData(res);
                        if (callData.result === 'success') {
            ...
            // } else {
            //     sendData['action'] = 'addRecommendCommodity';
            //     this.dataService.sendRequest(sendData).then(res => {
            //         if (res['serviceCall']) {
            //             const callData = this.dataService.getCallData(res);
            //             if (callData.result === 'success') {
